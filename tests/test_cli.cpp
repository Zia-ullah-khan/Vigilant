...
TEST_CASE("CLI Deploy fails when requested config directory is unusable", "[cli]")
{
    const auto root = test_helpers::MakeTempDir("cli_deploy_unusable");
    const auto sourceDir = root / "src";
    ...
    REQUIRE(rc != 0);
    REQUIRE_FALSE(std::filesystem::exists(root / "services" / "fallbacksvc.vig"));
